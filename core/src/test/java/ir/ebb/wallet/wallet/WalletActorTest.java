...
class WalletActorTest {
    ...
    private void createWallet() {
        testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.CreateWallet(USER.getDbsAccountNumber(), UUID.randomUUID(), ref));
    }

    @Test
    void createWallet_persistsCreatedEventAndEmptyState() {
        var res = testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.CreateWallet(USER.getDbsAccountNumber(), UUID.randomUUID(), ref));
    ...
    void deposit_afterCreate_persistsMutatedEventAndUpdatesBalance() {
        createWallet();
        var res = testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.Deposit(UUID.randomUUID(), USER.getDbsAccountNumber(), 100L, T0, TYPE, ref));
        assertThat(res.reply()).isInstanceOf(WalletReply.Accepted.class);
        assertThat(res.events()).hasSize(1);
    ...
    void duplicateTrackingId_isRejected() {
        ...
        createWallet();
        var first = testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.Deposit(tracking, USER.getDbsAccountNumber(), 100L, T0, TYPE, ref));
        assertThat(first.reply()).isInstanceOf(WalletReply.Accepted.class);

        var dup = testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.Deposit(tracking, USER.getDbsAccountNumber(), 50L, T0, TYPE, ref));
    ...
    void freeze_exceedingBalance_isRejectedInsufficient() {
        createWallet();
        var res = testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.Freeze(UUID.randomUUID(), USER.getDbsAccountNumber(), 100L, T0, TYPE, false, ref));
        assertThat(res.reply()).isInstanceOf(WalletReply.Rejected.class);
    ...
    void getWallet_afterDeposit_returnsSnapshotAndPersistsNothing() {
        createWallet();
        testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.Deposit(UUID.randomUUID(), USER.getDbsAccountNumber(), 100L, T0, TYPE, ref));
    ...
    void restart_recoversStateFromJournal() {
        createWallet();
        testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.Deposit(UUID.randomUUID(), USER.getDbsAccountNumber(), 100L, T0, TYPE, ref));
        testKit.runCommand((ActorRef<WalletReply> ref) -> new WalletCommand.Deposit(UUID.randomUUID(), USER.getDbsAccountNumber(), 30L, T0, TYPE, ref));
